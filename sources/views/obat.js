...
export default class Obat extends JetView {
		...
		var ui = {rows: [
			{ view:"template", template:"Data Jenis Penunjang", type:"header" },
			{
				...
				cols:[
					...
					{ view:"button", click:()=>this.ubahObat(), label:"Ubah", type:"iconButton", width:100 },
					{ view:"button", click:()=>this.hapusObat(), label:"Hapus", type:"iconButton", width:100 },
				]
			},
			{
				view:"datatable",
				...
				columns: [
					...
					{ id:"harga", header:["Harga",{content:"textFilter"}], width:100 },
		...
		return {
			...
			head:{
				view:"toolbar", margin:-4, cols:[
					...
					{ view:"button", type:"iconButton", label:"Tutup", width:80, click:"$$('windowFormObat').hide();"},
				]
			},
			body:{
				view:"form",
				...
				elements:[
					...
					{ cols: [
						{ template:"", borderless:true },
						{ view:"button", click:()=>this.simpanObat(), label:"Simpan", width:120, borderless:true },
						{ template:"", borderless:true },
					]}
				]
			}
	...
	refreshObat(){
		$$("tabelObat").clearAll();
		$$("tabelObat").load("http://localhost:3003/obats");
	}
	...
	simpanObat(){
		...
		if ($$("formObat").validate()) {
			...
			if (dataKirim.createdAt === undefined ) {
				webix.ajax().post("http://localhost:3003/obats", dataKirim, callbackHasil);
			} else {
				webix.ajax().put("http://localhost:3003/obats", dataKirim, callbackHasil);
			}
		}
	}
		...
		if (row) {
			...
			webix.confirm({
				...
				callback:function(jwb){
					if(jwb) {
						webix.ajax().del("http://localhost:3003/obats", row, callbackHasil);
					}
				}
			});
		}
