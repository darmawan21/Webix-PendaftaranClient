...
export default class TransaksiObatDetail extends JetView {
	config(){
		var ui = {rows: [
			{ view:"template", template:"Data Transaksi Bayar Detail", type:"header" },
			{
				...
				cols:[
					...
					{ view:"button", click:()=>this.cetakTagihanPdf(), label:"Cetak Kwitansi", type:"iconButton", width:200 },
			...
			},
			{
				...
				columns: [
					{ id:"id_transaksi_bayar", header:["Transaksi Bayar",{content:"textFilter"}], width:100 },
					{ id:"jumlah_bayar", header:["Jumlah Bayar",{content:"textFilter"}], width:300 },
	...
	formTransaksiObatDetail(){
		return {
			...
			body:{
				...
				borderless:true,
				elements:[
					{ view:"combo", label:"Transaksi Bayar", name:"id_transaksi_bayar", labelWidth:100, options: "http://localhost:3004/transaksi-bayar/options" },
					{ view:"text", label:"Jumlah_Bayar", name:"jumlah_bayar", labelWidth:100, require:true },
					{ cols: [
						{ template:"", borderless:true },
	...
	refreshTransaksiObatDetail(){
		...
		$$("tabelTransaksiObatDetail").load("http://localhost:3004/transaksi-bayar-detail");
	}
	...
	simpanTransaksiObatDetail(){
		...
		if ($$("formTransaksiObatDetail").validate()) {
			...
			if (dataKirim.createdAt === undefined ) {
				webix.ajax().post("http://localhost:3004/transaksi-bayar-detail", dataKirim, callbackHasil);
			} else {
				webix.ajax().put("http://localhost:3004/transaksi-bayar-detail", dataKirim, callbackHasil);
			}
		}
	}
		...
		if (row) {
			...
			webix.confirm({
				...
				callback:function(jwb){
					if(jwb) {
						webix.ajax().del("http://localhost:3004/transaksi-bayar-detail", row, callbackHasil);
					}
				}
			});
		}
	...
	cetakTagihan() {
		return {
			...
			template: `<table width='400'>
						<tr>
							<td>Transaksi Bayar : </td><td>#id_transaksi_bayar#</td>
						</tr>
						<tr>
							<td>Jumlah Bayar :</td><td>#jumlah_bayar#</td>
						</tr>
						<tr>
