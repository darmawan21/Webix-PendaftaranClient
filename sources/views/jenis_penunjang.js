...
export default class JenisPenunjang extends JetView {
	config(){
		var ui = {rows: [
			{ view:"template", template:"Data Jenis Penunjang", type:"header" },
			{
				...
				cols:[
					{ view:"button", click:()=>this.tambahJenisPenunjang(), label:"Tambah", type:"iconButton", width:100 },
					{ view:"button", click:()=>this.refreshJenisPenunjang(), label:"Refresh", type:"iconButton", width:100 },
					{ template:"", borderless:true },
					{ view:"button", click:()=>this.ubahJenisPenunjang(), label:"Ubah", type:"iconButton", width:100 },
					{ view:"button", click:()=>this.hapusJenisPenunjang(), label:"Hapus", type:"iconButton", width:100 },
				]
			},
			{
				view:"datatable",
		...
		return {
			...
			head:{
				view:"toolbar", margin:-4, cols:[
					{ view:"label", label: "Tambah", id:"judulFormJenisPenunjang" },
					{ view:"button", type:"iconButton", label:"Tutup", width:80, click:"$$('windowFormJenisPenunjang').hide();"},
				]
			},
			body:{
				view:"form",
				...
				elements:[
					...
					{ cols: [
						{ template:"", borderless:true },
						{ view:"button", click:()=>this.simpanJenisPenunjang(), label:"Simpan", width:120, borderless:true },
						{ template:"", borderless:true },
					]}
				]
			}
	...
	refreshJenisPenunjang(){
		...
		$$("tabelJenisPenunjang").load("http://localhost:3002/jenis-penunjang");
	}

	tambahJenisPenunjang(){
		$$("windowFormJenisPenunjang").show();
		$$("formJenisPenunjang").clear();
		$$("judulFormJenisPenunjang").setValue("Form Tambah JenisPenunjang");
	}
	...
	ubahJenisPenunjang(){
		...
		if (row) {
			...
			$$("judulFormJenisPenunjang").setValue("Form Ubah JenisPenunjang");
		}
		else {
	...
	simpanJenisPenunjang(){
		...
		if ($$("formJenisPenunjang").validate()) {
			...
			if (dataKirim.createdAt === undefined ) {
				webix.ajax().post("http://localhost:3002/jenis-penunjang", dataKirim, callbackHasil);
			} else {
				webix.ajax().put("http://localhost:3002/jenis-penunjang", dataKirim, callbackHasil);
			}
		}
	}
		...
		if (row) {
			...
			webix.confirm({
				...
				callback:function(jwb){
					if(jwb) {
						webix.ajax().del("http://localhost:3002/jenis-penunjang", row, callbackHasil);
					}
				}
			});
		}
